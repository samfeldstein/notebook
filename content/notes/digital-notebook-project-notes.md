...
This only works if you're okay with keeping your private notes in a specific folder. A more flexible approach would be to include a `private` key in a note's frontmatter, and use that to filter it from git and the site output. Though as you can see in my [notes on the subject](notes/private-notes-in-eleventy.md), this is more complicated than it sounds.
